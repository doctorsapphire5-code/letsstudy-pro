...
if (!orderId) {  
  ...
  max 50 characters and limited characters.  
  */  
  ...
  */  
  ...
  const payload = {  
    ...
    billing_address: {  
      ...
      phone_number: clean(phoneNumber),  
  ...
  /*  
  This redirect_url is the Pesapal iframe URL.  
  */  
...
const paymentStatus =  
  getPesapalStatusName(statusCode);  
...
=====================================================  
IMPORTANT:  
This is where your Firestore order/payment update  
should happen.  

COMPLETED = mark order as paid  
FAILED    = mark payment failed  
REVERSED  = mark payment reversed  
=====================================================  
*/  
...
clean(req.query.orderTrackingId);
...
Do not trust callback as proof of payment.  
*/  
...
/*  
Customer is sent back to your website.  
*/  
const verifyUrl =  
...
/*
This runs independently of the customer's browser.
*/
...
/*  
Pesapal accepts either GET or POST depending  
on how the IPN was registered.  
*/  
res.status(200).json({  
  orderNotificationType:  
    "IPNCHANGE",  
...
MANUAL BACKGROUND VERIFY

Useful for verify.html or admin.
...
if (!orderTrackingId) {  
  ...
  const result =  
    await verifyPaymentInBackground(  
      clean(orderTrackingId),  
      clean(merchantReference),  
      "MANUAL"  
    );  
